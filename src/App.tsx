...
import { loadCSV, loadCSVFromFile, AVAILABLE_STOCKS, loadAvailableStocks } from './services/csvLoader';
...
const App: React.FC = () => {
  ...
  const [uploadedCsvNames, setUploadedCsvNames] = useState<{ primary: string | null; fine: string | null }>({ primary: null, fine: null });
  const [customCsvData, setCustomCsvData] = useState<Record<string, OHLCVBar[]>>({});
  ...
  const handleUploadCsv = useCallback(async (file: File, timeframe: 'primary' | 'fine') => {
    try {
      setLoading(true);
      setError(null);
      const cleanedName = file.name.replace(/\.csv$/i, '').trim() || 'uploaded_csv';
      const data = await loadCSVFromFile(file);

      setCustomCsvData((previous) => ({ ...previous, [cleanedName]: data }));
      setAvailableStocks((previous) => {
        const next = [...new Set([...previous, cleanedName])].sort((a, b) => a.localeCompare(b));
        return next;
      });
      setUploadedCsvNames((previous) => ({ ...previous, [timeframe]: cleanedName }));

      if (timeframe === 'primary') {
        setSelectedStock(cleanedName);
      } else {
        setFineStock(cleanedName);
      }

      setLoading(false);
      resetSelection();
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to read the uploaded CSV.';
      setError(message);
      setLoading(false);
    }
  }, [resetSelection]);
  ...
  useEffect(() => {
    ...
    const primaryData = customCsvData[selectedStock] ?? null;
    const fineDataForSelection = customCsvData[fineStock] ?? null;

    Promise.all([
      primaryData ? Promise.resolve(primaryData) : loadCSV(selectedStock),
      fineDataForSelection ? Promise.resolve(fineDataForSelection) : loadCSV(fineStock),
    ])
      .then(([primary, fine]) => {
  ...
  }, [selectedStock, fineStock, customCsvData, resetSelection]);
      ...
      <div className="main-layout" style={{ '--sidebar-width': `${sidebarWidth}px` } as React.CSSProperties}>
        <div className="workspace-column">
          <AnalyticsSetup
            ...
            uploadedCsvNames={uploadedCsvNames}
            disabled={loading}
          />
