...
const ChartPanel: React.FC<ChartPanelProps> = ({
    ...
    return (
        <div className={`chart-panel-wrapper ${minimized ? 'chart-panel-minimized' : ''}`}>
            {label && (
                <div className="chart-panel-header">
                    ...
                    <div className="chart-panel-actions">
                        <button
                            ...
                            onClick={onMinimize}
                            aria-label={minimized ? `Restore ${label}` : `Minimize ${label}`}
                            title={minimized ? `Restore ${label}` : `Minimize ${label}`}
                        >
                            {minimized ? '▢' : '—'}
                        </button>
                        <button
                            type="button"
                            className="chart-panel-button chart-panel-close"
                            onClick={onClose}
            ...
            <div className="chart-panel-body">
                {minimized && <div className="chart-panel-minimized-state">Panel minimized — click restore</div>}
                <div ref={containerRef} className="chart-container" style={{ display: minimized ? 'none' : 'block' }} />
            </div>
        </div>
    );
};
